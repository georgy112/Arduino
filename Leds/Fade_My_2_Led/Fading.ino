/*
  Плавно моргаем светодиодом на 9 8 пинах, но красный все еще резок.
  Fading
...
void loop() {
  ...
  for (int fadeValue = 0 ; fadeValue <= 130; fadeValue += 1) {
  ...
  for (int fadeValue = 130 ; fadeValue >= 0; fadeValue -= 1) {
    // sets the value (range from 0 to 255):
    analogWrite(ledBlue, fadeValue);
    analogWrite(ledRed, fadeValue);
    // wait for 30 milliseconds to see the dimming effect
